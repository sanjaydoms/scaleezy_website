...
export function Stories() {
  return (
    <section id="stories" className="w-full bg-cream py-24 text-forest md:py-36">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          {STORIES.map((story, index) => (
            <Reveal
              key={story.name}
              delay={index * 0.1}
              className="flex flex-col overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-forest/5"
            >
              <div className="group relative aspect-[4/3] w-full overflow-hidden">
                ...
                <button
                  type="button"
                  aria-label={`Play video story for ${story.name}`}
                  className="absolute left-1/2 top-1/2 flex h-14 w-14 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-forest text-cream shadow-lg transition-transform hover:scale-110 focus:outline-none focus-visible:ring-2 focus-visible:ring-lime focus-visible:ring-offset-2"
                >
