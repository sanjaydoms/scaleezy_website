...
export function Platform() {
  ...
  useEffect(() => {
    const updateRange = () => {
      if (railRef.current) {
        setScrollRange(
          Math.max(0, railRef.current.scrollWidth - window.innerWidth)
        );
      }
    };
    updateRange();
  ...
  if (reduceMotion) {
    return (
      <section
        id="platform"
        className="w-full overflow-hidden bg-cream py-24 text-forest md:py-36">
  ...
  return (
    <article
      ...
      <div className="relative z-10 flex h-full w-full flex-col p-7 md:p-8">
        ...
        <div
          className={`mt-5 ${compact ? '' : 'transition-transform duration-300 ease-out group-hover:-translate-y-1'}`}>
