...
export function Industries() {
  return <section id="industries" className="w-full bg-forest py-24 text-cream md:py-36">
      <div className="mx-auto max-w-[1600px] px-6 md:px-12">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {INDUSTRIES.map((industry, index) => <Reveal key={industry.name} delay={index * 0.05} className="group relative aspect-[4/5] overflow-hidden">
              <img src={industry.image} alt={industry.alt} loading="lazy" className="h-full w-full object-cover transition-transform duration-[1200ms] ease-out group-hover:scale-105 group-focus-within:scale-105" />
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 transition-opacity duration-300 group-hover:opacity-0 group-focus-within:opacity-0">
                ...
                <h3 className="font-serif-display mt-3 text-4xl leading-none">
                  {industry.name}
                </h3>
              </div>
              <div className="absolute inset-0 flex flex-col justify-end p-6 opacity-0 transition-opacity duration-500 group-hover:opacity-100 group-focus-within:opacity-100">
                ...
                <p className="mt-4 text-base font-light leading-relaxed text-cream/70">
                  {industry.challenge}
                </p>
                <p className="mt-5 border-t border-cream/25 pt-4 font-serif-display text-2xl leading-tight">
                  {industry.response}
                </p>
              </div>
              <button type="button" aria-label={`${industry.name}: ${industry.challenge} ${industry.response}`} className="absolute inset-0 h-full w-full cursor-default focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-lime" />
            </Reveal>)}

          <Reveal delay={0.35} className="flex flex-col justify-center bg-white p-8 text-forest sm:col-span-2 lg:col-span-2">
