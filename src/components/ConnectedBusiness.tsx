...
export function ConnectedBusiness() {
  return <section className="w-full bg-[#121712] py-24 text-cream md:py-36">
      <div className="mx-auto max-w-[1600px] px-6 md:px-12">
        ...
        <div className="mt-16 grid items-center gap-8 lg:grid-cols-[1fr_auto_1fr] lg:gap-12">
...
function SignalList({
  ...
  return <Reveal className={align === 'right' ? 'lg:text-right' : ''}>
      <p className="eyebrow text-cream/45">{heading}</p>
      <ul className="mt-6 space-y-3">
        {items.map((item) => <li key={item} className="border-b border-cream/15 pb-3 text-xl font-light text-cream/80">
            {item}
          </li>)}
      </ul>
    </Reveal>;
